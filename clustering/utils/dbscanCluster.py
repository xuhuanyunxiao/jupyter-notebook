...
def getReturnData(label,messageObjList):
    ...
    for line in noImpurity:
        ...
        if label not in noImpurityGroup.keys():
            ...
            noImpurityGroup[label]["mlcpersite"][site_name] = {'content': content, 'maxlength':  content_length, 'id': id}
        else:
            noImpurityGroup[label]["id"].append(id)
...
def getReturnDataWithId(label,messageObjList):
    ...
    for (label, value) in noImpurityGroup.items():
        ...
        # 构造ClusterObj
        cObj = {"clusterId": clusterId, "clusterTopic": clusterTopic, "clusterPublishtimeRange": clusterPublishtimeRange, "clusterMember": clusterMember, "clusterMessageId": clusterMessageId}
        cObjArray.append(cObj)
    return cObjArray
