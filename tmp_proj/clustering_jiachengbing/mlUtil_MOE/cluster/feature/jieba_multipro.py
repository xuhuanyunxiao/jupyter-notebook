...
def clear_sen(sent):
    ...
    reobj = re.compile(
        "((([hH][tT][tT][pP]([sS]?):\\/\\/)|(([wW][wW][wW]\\.)|([wW][aA][pP]\\.)))(([\\.\\-\\w]{3,})(:\\d*)?)([\\/?\\(\\)\\?|\\w|\\-|~|=|\\&|\\%+\\.]+))")
    sent = reobj.sub("", sent)
...
def cut_tasks(articles, num_tasks):
    ...
    logger.debug("starting cut_tasks, {articles, %d, num_tasks, %d}." % (len(articles), num_tasks))
    task_list = []
    ...
    logger.debug("ending cut_tasks, {cost_time: %ds}" % (time.time() - start_time))
    return task_list
...
def multi_process(processes, task_list, run, attach):
    ...
    """
    start_time = time.time()
    logger.debug("starting multi_process, {processes: %d, task_list: %d }." % (processes, len(task_list)))
    ...
    logger.debug("ending multi_process, {task_time: %ds}" % (time.time() - start_time))
    return cut_result


if __name__ == '__main__':
    articles = [('南通加大澳門聯繫力度貢獻國',
                 '【本報訊】澳門南通商會第二屆理監事暨澳門南通聯誼會第一屆理監事就職典禮昨晚舉行。會長施春雷表示，當前國家經濟社會發展進入新時代，南通正努力建設上海北大門，做好「跨江融合、接軌上海」的大文章，而澳門正積極融入國家發展大局，建設蘇澳合作園區，促進經濟適度多元發展，澳門與南通的經貿往來、人員往來將會更加緊密。他期望商會和聯誼會能夠建設成聯繫政府與企業、企業與企業、企業與鄉親的橋樑和紐帶，為澳門和南通兩地的經濟發展做出更大貢獻。'),
                ('你所知道的背影（一）　紹　鈞',
                 '新近閱讀了一篇有關艾未未的文章，該文陳述他將赴柏林藝術大學擔任客席教授。另文中也提及，他對於教授藝術的一些觀念，像認為藝術「已經不再只是關乎形式與形狀，而是更關乎於哲學和社會政治觀點。」'),
                ('感謝她盛裝蒞臨我的青春　陳　穎',
                 '我小時候有一個夢想，就是可以穿上灰姑娘的水晶鞋，坐上南瓜車，搖曳着華麗的裙擺來到舞會，成為當刻最耀眼的那個公主。眾所周知，灰姑娘的美麗背後，有一位惡毒的繼母和兩個心地不好的姐姐，她們處處刁難她，每天讓她做粗重的工作。也正因為這個故事，繼母這狠角色已植根在我的腦海中。'),
                ('市民微信亂講嘢或觸內地法律',
                 '【本報訊】政府昨日舉行最後一場《網絡安全法》公開諮詢，郵電局資訊科技發展處處長李廣亮稱，微信、Whatsapp等不屬本地區的軟件，將來法律未必可以規管；司警局電腦法證處處長陳思晶稱，每個人要為自己的言論負責，倘如在微信「亂講嘢」可能觸犯內地網安法。'),
                ('')]
    num_processes = 2
