...
class SubjectManualInfo(BaseMysql):
    ...
    def read_jobs(self, group_id):
        ...
        """
        sql = """
        SELECT group_id, id AS subject_id, 
    ...
    def update_perform_status(self, subject_id, perform_status):
        """ 更新job状态: 是否执行：1-未执行；2-正在执行；3-已执行
        ...
        # logger.info('update_perform_status. id: %s; perform_status: %s .%s' % (subject_id, perform_status, str(sql)))
        param = [(perform_status, subject_id)]

        self.execute_many_with_transaction(sql, param)
