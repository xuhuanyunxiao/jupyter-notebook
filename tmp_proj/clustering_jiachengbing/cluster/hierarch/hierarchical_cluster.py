...
def first_get_return_data(labels, corpus, keywords_list = None):
    ...
    for label, value in purity_group.items():
        ...
        if keywords_list:
            keywords = keywords_list[label + 10000]  # 与 ind - 10000 一致
            word_freq_head = [(word, len(title_list)) for word in keywords]
            key_word_str = '/'.join(keywords)
            key_word_str = '(' + key_word_str + ')'
        else :
...
import nltk
def decide_contents(keywords, word_list):
    """
    判断文章中是否含有一组关键词
    """
    kw_flag = False
    for kk in keywords:
        intersection = set(kk).intersection(set(word_list))
        if len(kk) == len(intersection):
            kw_flag = True
    return kw_flag
...
def perform_cluster(is_manual, cluster_type, manual_id, subtopic_id, language_type,
    ...
    keywords_list = base_data_view.get_keywords_for_sel_clusterdata(language_type)
    keywords_list_title = [k[0].split(';') for k in keywords_list]
    keywords_list_match = [k[1] for k in keywords_list]
    logger.info('get_keywords_for_sel_clusterdata. keywords num: %s; keywords[0]: %s'%(
                                                    len(keywords_list), keywords_list[0]))
    ...
    for index, obj in enumerate(corpus):
        ...
        word_list = nltk.word_tokenize(contents.decode('utf-8'))
        flag = 0
        for ind, keywords in enumerate(keywords_list_match):
            kw_flag = decide_contents(keywords, word_list)
            if kw_flag:  # True
                keyword_clusterdata.append(obj)
                keyword_label.append(ind - 10000)
                flag = 1
                break
        if flag == 0:
            first_articles.append((title, content))
            corpus_sel.append(obj)
    first_keywords_result = first_get_return_data(keyword_label, keyword_clusterdata,
                                                  keywords_list=keywords_list_title)
    logger.info('first_keywords_result. data num: %s; label num: %s; lables: %s' % (
                    len(keyword_label), len(first_keywords_result), first_keywords_result.keys()))

    """  第一步, 根据标题, 内容聚类  """  # corpus 替换成 corpus_sel
    first_articles = [(obj.messageTitle, obj.messageContent) for obj in corpus_sel]
    first_cut_results = nltk_multipro_new.multi_cut_words(first_articles,
                                                          num_processes)
    logger.info('first_cut_results end....')

    ziped_first_corpus_cut_results = zip(corpus_sel, first_cut_results)
    ziped_first_corpus_cut_results = filter(lambda t: t[1] != '', ziped_first_corpus_cut_results)
    corpus_sel = [t[0] for t in ziped_first_corpus_cut_results]
    ...
    first_cluster_results = first_get_return_data(first_labels, corpus_sel)
    logger.info('first_cluster_results end....')

    # 合并两次结果，作为第一步聚类结果
    logger.info('Combined first cluster results...')
    for key, value in first_keywords_result.items():
        if key not in first_cluster_results:
            first_cluster_results[key] = value
        else :
            logger.info('label error on first_keywords_result')
    ...
    if len(first_cluster_results) == 0:
        ...
        # 二次聚类, 根据topic
        # label ＜ 0， 关键词选出的数据
        second_keyword_label = []
        second_corpus = []
        for (label, topic) in first_cluster_results.items():
            if label < 0:
                second_keyword_label.append(label)
            else :
                second_corpus.append((label, topic['title_list']))
        ...
        # 合并关键词的label
        for keyword_label in second_keyword_label:
            # logger.info('keyword_label：%s' % (keyword_label))
            ids_list.append([keyword_label])
