...
def return_data(cluster_ids_list, first_cluster_results, min_sample):
    ...
    for idx, cluster_ids in enumerate(cluster_ids_list):
        ...
        for cluster_id in cluster_ids:
            ...
            _zipped_ids_publistimes = zip(_ids, _publishtimes)
            _zipped_ids_publistimes.sort(key=lambda t:t[1], reverse=True)
            _ids = [t[0] for t in _zipped_ids_publistimes]
        ...
        # clusterMember
        ids.sort(key=lambda members:len(members), reverse=True)
